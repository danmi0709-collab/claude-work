"""용인수지구 탭 생성 + 전고전저 채우기
사용:
  1단계(탭생성):   python 용인수지구_시세탭.py tab
  2단계(데이터수집): python 용인수지구_시세탭.py fetch
  3단계(전고전저):  python 용인수지구_시세탭.py high
  전체:           python 용인수지구_시세탭.py all
"""
...
SRC  = os.path.join(BASE, '용인시 수지구.xlsx')
DATA = os.path.join(BASE, '용인수지구_시세')
