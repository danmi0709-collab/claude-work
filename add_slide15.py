...
"""
슬라이드 15 가격박스 추가
- 코드: 174198, 3751, 111515, 1196, 348, 595, 12948, 610
- 폰트 10pt
- 입력/출력: CLAUDE 폴더 PPT (사용자가 직접 코드 삽입한 파일)
"""
    ...
    for key in SEARCH_KEYS:
        if key in apt_name:
            for html_name, lines in extra_db.items():
                if key in html_name:
                    # 연식/세대수처럼 보이는 항목 필터링
                    filtered = [l for l in lines if not re.match(r"^\d{2}'\s*[\d,]+\^", l)]
                    return filtered
    return []

# ── Excel 로드 ─────────────────────────────────────────────────
NEW_CODES = [174198, 3751, 111515, 1196, 348, 595, 12948, 610]
...
for code in NEW_CODES:
    ...
    if rows:
        r0 = rows[0]
        print(f'  [{code}] {r0["name"]} {r0["year"]} {r0["sedae"]}^')
    else:
        print(f'  [{code}] 데이터 없음')
...
slide = prs.slides[14]   # 슬라이드 15 = index 14
...
print(f'코드 박스 {len(delete_elems)}개 삭제')
