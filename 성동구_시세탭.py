"""성동구 탭 생성 + 데이터 수집 + 전고전저"""
import sys, io, os, re, json, csv, time, datetime
...
SRC   = os.path.join(BASE, '성동구.xlsx')
DATA  = os.path.join(BASE, '성동구_시세')
...
def fetch_all(ymd):
    ...
    while True:
        ...
        u=URL_T+'?'+urllib.parse.urlencode(p,safe=':/')
...
for r in range(2,src_ws.max_row+1):
    ...
    rows.append({'dong':src_ws.cell(r,3).value,'code':code,'name':src_ws.cell(r,6).value,
        'approved':src_ws.cell(r,7).value,'hh_tot':src_ws.cell(r,8).value,
        'area_excl':src_ws.cell(r,9).value,'struct':src_ws.cell(r,10).value,
        'rooms':src_ws.cell(r,11).value,'baths':src_ws.cell(r,12).value,
        'type_nm':src_ws.cell(r,13).value,'hh_type':src_ws.cell(r,14).value})
print(f'성동구.xlsx: {len(rows)}행')

wb=openpyxl.load_workbook(FILE)
if '성동구' in wb.sheetnames: del wb['성동구']
ws=wb.copy_worksheet(wb['창원성산구'])
ws.title='성동구'
for r in range(5,ws.max_row+1):
    for c in range(1,42):
        ws.cell(r,c).value=None; ws.cell(r,c).hyperlink=None; ws.cell(r,c).fill=WHITE
...
for i,d in enumerate(rows):
    ...
    nc.font=Font(name=nc.font.name,size=nc.font.size,bold=nc.font.bold,color='0563C1',underline='single')
    ...
    ws.cell(r,13,value=d['struct']); ws.cell(r,14,value=d['rooms'])
    ws.cell(r,15,value=d['baths']); ws.cell(r,16,value=d['hh_type'])

prev_key=None; use_gray=False
for i,d in enumerate(rows):
    r=5+i
    key=(d['dong'],d['name'],d['approved'])
    if key!=prev_key: use_gray=not use_gray; prev_key=key
    if use_gray:
...
def best_match(dong,byear,ar,name,tol=1):
    ...
    if sub:
        top=Counter(c['apt'] for c in sub).most_common(1)[0][0]; return [c for c in sub if c['apt']==top]
    top=Counter(c['apt'] for c in cand).most_common(1)[0][0]; return [c for c in cand if c['apt']==top]

wb=openpyxl.load_workbook(FILE); ws=wb['성동구']
high_n=low_n=proc=0
for r in range(5,ws.max_row+1):
