"""
천안동남구 탭 AA~AL 일괄 채우기
- 데이터 범위: 2018-01 ~ 2026-04
- AA: 매수시기(80%+ 기간 중 최저매매 월)
- AB: 기간 길이
- AC: 평형 수식 =L{r}
- AD: 매수매매가
- AE: 매수전세가
...
# 천안동남구 컬럼 번호
C_DONG, C_NAME, C_YEAR, C_AREA_L = 1, 5, 6, 12  # L=평형(12), K=전용면적(11)
C_PYEONG_INPUT = 12  # L 평형
C_AREA = 11  # K 전용면적
C_Q = 17
C_AA = 27; C_AB = 28; C_AC = 29; C_AD = 30; C_AE = 31; C_AF = 32; C_AG = 33
C_AH = 34; C_AI = 35; C_AJ = 36; C_AK = 37; C_AL = 38
...
for r in range(5, ws.max_row + 1):
    ...
    # AC 평형 수식
    ws.cell(r, C_AC, value=f'=L{r}')
    # AF 전세가율 = AE/AD (수식, 값은 엑셀에서 계산)
    ws.cell(r, C_AF, value=f'=AE{r}/AD{r}')
    # AG 투자금 = AD - AE
    ws.cell(r, C_AG, value=f'=AD{r}-AE{r}')
    # AH 매도시기 고정
    ws.cell(r, C_AH, value=2026.04)
    # AI 현 매매가 = Q값 복사
    q_val = ws_ro.cell(r, C_Q).value
    if q_val is None:
        q_val = ws.cell(r, C_Q).value
    if q_val is not None:
        ws.cell(r, C_AI, value=q_val)
        filled_ai += 1
    # AJ, AK, AL 수식
    ws.cell(r, C_AJ, value=f'=AI{r}-AD{r}')
    ws.cell(r, C_AK, value=f'=1+(AI{r}-AD{r})/AD{r}')
    ws.cell(r, C_AL, value=f'=AJ{r}/AG{r}')

wb.save(FILE)
print(f'처리 {proc}행, AA기간 채움 {filled_aa}, AI(현매매가) 채움 {filled_ai}')
