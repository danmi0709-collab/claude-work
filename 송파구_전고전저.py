"""송파구 탭 전고(U)/전저(V) 채우기
- 전고: 2021.01 ~ 2022.12 최고 매매가 → U열 (억 단위)
- 전저: 2022.01 ~ 2023.12 최저 매매가 → V열 (억 단위)
"""
...
CSV  = os.path.join(BASE, '송파구_시세', '매매_원본.csv')
...
# A=동(1), E=단지명(5), F=사용승인일(6), K=전용면적(11), U=전고(21), V=전저(22)
high_n = low_n = proc = 0
for r in range(5, ws.max_row + 1):
    dong  = ws.cell(r, 1).value
    name  = ws.cell(r, 5).value
    y_val = ws.cell(r, 6).value
    a_val = ws.cell(r, 11).value
    if not (dong and name and y_val and a_val): continue
    try:
        byear = int(float(str(y_val)[:4]))
    ...
    if high is not None:
        ws.cell(r, 21, value=round(high / 10000, 2))
        high_n += 1
    if low is not None:
        ws.cell(r, 22, value=round(low / 10000, 2))
        low_n += 1
